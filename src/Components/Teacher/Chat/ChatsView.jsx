...
const ChatsView = () => {
  ...
  const chatList = useSelector((state) => state?.teacherChatData?.chatList);

  const filteredChatList = chatList.filter((user) =>
    user.studentInfo?.fullName.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  useEffect(() => {
    socket.emit("register", teacherData._id);

    const headers = {
      Authorization: localStorage.getItem("teacherToken"),
    };

    getStudentsApi(headers).then((response) => setStudents(response.data));
    dispatch(fetchChatList(teacherData._id));

    return () => {
      socket.off("recieve-message");
    };
  }, [dispatch, teacherData._id]);

  useEffect(() => {
    socket.on("recieve-message", (incomingMessage) => {
      dispatch(addMessageToChatList(incomingMessage));
      if (incomingMessage.senderId === activeChat.studentId) {
            ...
            activeChat ? (
              <div className="flex items-center">
                ...
                <img
                  ...
                  alt={activeChat.studentInfo?.fullName}
                  className="h-10 w-10 rounded-full mr-2 shadow-sm"
                />
